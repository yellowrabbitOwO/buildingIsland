...
import type { ScriptDoc, ScriptFolder } from "../../data/types";
import { deleteScriptDoc, duplicateScriptDoc, moveScriptDocsToFolder, toggleScriptDocStar } from "../../data/repositories/scriptDoc";
...
export default function ScriptDocCard({ doc, worldId, folders, dragProps }: ScriptDocCardProps) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (ok) await deleteScriptDoc(doc.id);
  };
  ...
  return (
    <div
      ...
      onClick={() => navigate(`/world/${worldId}/script/${doc.id}`)}
      {...dragProps}
    >
      {doc.tagColor && (
      ...
      <div style={{ padding: 12, display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            openPanel({ kind: "scriptDoc", docId: doc.id });
          }}
        >
          ⇲
        </button>
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            duplicateScriptDoc(doc.id, undefined, t);
          }}
        >
          ⧉
        </button>
        ...
        <Modal title={t("entryCard.moveToFolder")} onClose={() => setShowMove(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }} onClick={(e) => e.stopPropagation()}>
            <button
              ...
              onClick={async () => {
                await moveScriptDocsToFolder([doc.id], undefined);
                setShowMove(false);
              }}
            >
              {t("entryCard.unfiled")}
            ...
            {(folders ?? []).map((f) => (
              <button
                ...
                onClick={async () => {
                  await moveScriptDocsToFolder([doc.id], f.id);
                  setShowMove(false);
                }}
              >
                {f.name}
